...
public class TrackItemData {
    ...
    public static final class ToTrackDtoMapper implements Function<TrackItemData, TrackDto> {

        private final String playingFrom;

        @Override
        public @NonNull TrackDto apply(@NonNull TrackItemData item) {
            return new TrackDto(item.id, item.title, item.artistId, item.artistName,
                    item.albumId, item.albumTitle, item.duration,
                    item.uri, item.artworkPath, playingFrom);
        }
