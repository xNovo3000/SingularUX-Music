...
public class OnTimelineActionUseCase {
    ...
    public @NonNull Completable run(@NonNull TimelineAction action) {
        Log.d(TAG, "Running action " + action);
        if (action instanceof TimelineAction.ReplaceMediaItems) {
            return run((TimelineAction.ReplaceMediaItems) action);
        } else if (action instanceof TimelineAction.AddToCustomQueue) {
            return run((TimelineAction.AddToCustomQueue) action);
        }
        throw new IllegalArgumentException("Invalid action " + action);
    }

    private @NonNull Completable run(@NonNull TimelineAction.ReplaceMediaItems action) {
    ...
    private @NonNull Completable run(@NonNull TimelineAction.AddToCustomQueue action) {
